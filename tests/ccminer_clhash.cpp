/*
 * CCMiner CLHash v2.2 - Extracted for comparison testing
 * 
 * This is a direct copy from /tmp/ccminer-verus/verus/verus_clhash.cpp
 * with renamed function to avoid conflicts with our implementation.
 */
...
// Include our haraka.h for the macros and types
#include "../src/crypto/haraka.h"

extern "C" {

// Renamed version of ccminer's lazyLengthHash
static inline __m128i ccminer_lazyLengthHash(uint64_t keylength, uint64_t length) {
...
// Renamed version of ccminer's precompReduction64
static inline uint64_t ccminer_precompReduction64(__m128i A) {
    const __m128i C = _mm_cvtsi64_si128((1U << 4) + (1U << 3) + (1U << 1) + (1U << 0));
    const __m128i Q2 = _mm_clmulepi64_si128(A, C, 0x01);
    const __m128i Q3 = _mm_shuffle_epi8(_mm_setr_epi8(0, 27, 54, 45, 108, 119, 90, 65, 
...
// Direct copy of ccminer's __verusclmulwithoutreduction64alignedrepeatv2_2
// with renamed function
static __m128i ccminer_clmul_v2_2(__m128i *randomsource, const __m128i buf[4], uint64_t keyMask,
    uint32_t *fixrand, uint32_t *fixrandex, u128 *g_prand, u128 *g_prandex)
{
    ...
    for (int64_t i = 0; i < 32; i++)
    {
        ...
        case 0x14:
        {
            // we'll just call this one the monkins loop
...
// The full ccminer verusclhashv2_2 function (renamed)
uint64_t ccminer_verusclhashv2_2(void *random, const unsigned char buf[64], uint64_t keyMask,
    uint32_t *fixrand, uint32_t *fixrandex, u128 *g_prand, u128 *g_prandex)
{
    __m128i acc = ccminer_clmul_v2_2((__m128i *)random, (const __m128i *)buf, 511, 
                                      fixrand, fixrandex, g_prand, g_prandex);
    acc = _mm_xor_si128(acc, ccminer_lazyLengthHash(1024, 64));
    return ccminer_precompReduction64(acc);
}
